...
class LWEKeyExchange:
    ...
    def setup(self):
        ...
        # skip full rank check — a random 512×256 binary matrix is virtually always full rank
    ...
    def run_batch(self, bs):
        ...
        # maj() for all bs runs at once
        nz1 = (K1.reshape(bs, -1) != 0).sum(dim=1)
        nz2 = (K2.reshape(bs, -1) != 0).sum(dim=1)
        maj1 = nz1 >= self.l / 2
        maj2 = nz2 >= self.l / 2

        return (maj1 == maj2).sum().item()

    def experiment(self, num_experiments=10000, num_runs=10000, batch_size=500):
        results = []
